...
type Sector = "peluqueria" | "restaurante" | "cafeteria" | "fisio" | "estetica" | "bar";
...
export const Route = createFileRoute("/plantillas/$slug")({
  loader: ({ params }) => {
    const data = SECTORS[params.slug as Sector];
    if (!data) throw notFound();
    return { sector: params.slug as Sector, data };
  },
  head: ({ loaderData }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `Demo navegable de una web para ${loaderData?.data.label?.toLowerCase() ?? "este sector"}. Plantilla ficticia de mitxoko.`,
      },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center flex-col gap-4 bg-background">
      ...
      <Link to="/plantillas" className="link-underline">
...
function SectorPage() {
  ...
  return (
    <div style={{ background: data.bg }}>
      {/* Demo banner */}
      <div className="bg-primary text-primary-foreground px-6 lg:px-10 py-3 flex flex-wrap items-center justify-between gap-3 text-sm sticky top-0 z-50">
        <span>
          Estás viendo una <strong>plantilla demo</strong> · negocio ficticio
        </span>
        <Link
          to="/contacto"
          className="inline-flex items-center gap-2 bg-primary-foreground text-primary px-4 py-1.5 rounded-full hover:opacity-90 transition-opacity font-medium"
        >
      ...
      >
        ...
        <nav className="hidden md:flex gap-8 text-sm" style={{ color: `${data.accent}aa` }}>
          <span>Inicio</span>
          <span>Servicios</span>
          <span>Galería</span>
          <span>Contacto</span>
      ...
      <section className="relative">
        <div className="absolute inset-0">
          ...
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${data.accent}cc 0%, ${data.accent}66 100%)` }}
          />
        </div>
        <div className="relative px-6 lg:px-10 py-32 md:py-48 max-w-6xl mx-auto text-white">
          <motion.p
          ...
          >
            <button className="bg-white text-foreground px-6 py-3 rounded-full text-sm font-medium">
              Reservar cita
            </button>
            <button className="border border-white/40 text-white px-6 py-3 rounded-full text-sm">
              Ver servicios
            </button>
          </motion.div>
        </div>
      ...
      <section className="px-6 lg:px-10 py-20 max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-x-12 gap-y-8">
          {data.services.map((s: { name: string; price: string; desc: string }, i: number) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className="flex items-baseline justify-between gap-4 mb-2">
                ...
                <span className="font-display text-xl" style={{ color: data.accent }}>
                  {s.price}
                </span>
              </div>
      ...
      {/* Galería */}
      <section className="px-6 lg:px-10 py-20 max-w-6xl mx-auto">
        <h2 className="font-display text-4xl md:text-5xl mb-12 text-foreground">Galería</h2>
      ...
      {/* Sobre nosotros */}
      <section className="px-6 lg:px-10 py-20" style={{ background: `${data.accent}10` }}>
        <div className="max-w-4xl mx-auto">
          <h2 className="font-display text-4xl md:text-5xl mb-6 text-foreground">
            Sobre nosotros
          </h2>
          <p className="text-lg text-foreground/75 leading-relaxed">
            Llevamos más de quince años en el barrio. Empezamos pequeño y crecimos porque
            nuestros clientes nos recomendaron. Nuestro trato es cercano y profesional. Si
            vienes una vez, ya somos un poco amigos. Te esperamos.
          </p>
          <div className="grid sm:grid-cols-3 gap-4 mt-10">
            {[1, 2, 3].map((_, i) => (
              ...
              >
                <div className="flex gap-1 mb-3" style={{ color: data.accent }}>
                  {[...Array(5)].map((_, j) => (
                    <Star key={j} size={14} fill="currentColor" />
                  ))}
                </div>
                <p className="text-sm text-foreground/75 italic">
                  "Trato espectacular y precio justo. Vengo siempre que puedo."
                </p>
                <p className="text-xs mt-3 text-foreground/50">— Cliente habitual</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 lg:px-10 py-20 max-w-6xl mx-auto grid md:grid-cols-2 gap-12">
        <div>
          <h2 className="font-display text-4xl mb-8 text-foreground">Visítanos</h2>
          <div className="space-y-4">
            <p className="flex items-start gap-3">
              <MapPin size={18} style={{ color: data.accent }} className="mt-1" />
              <span>Calle Estafeta 42, 31001 Pamplona</span>
            </p>
            <p className="flex items-start gap-3">
              <Phone size={18} style={{ color: data.accent }} className="mt-1" />
              <span>948 12 34 56</span>
            </p>
            <p className="flex items-start gap-3">
              <Mail size={18} style={{ color: data.accent }} className="mt-1" />
              <span>hola@negocio.es</span>
            </p>
            <p className="flex items-start gap-3">
              <Instagram size={18} style={{ color: data.accent }} className="mt-1" />
              <span>@negocio</span>
            </p>
          </div>
          <h3 className="font-display text-2xl mt-10 mb-4 text-foreground flex items-center gap-2">
        ...
        <div>
          <h2 className="font-display text-4xl mb-8 text-foreground">Escríbenos</h2>
          <form className="space-y-4">
            <input
              className="w-full bg-white border border-border rounded px-4 py-3 text-sm"
              placeholder="Nombre"
            />
            <input
              className="w-full bg-white border border-border rounded px-4 py-3 text-sm"
            ...
            <button
              type="button"
              className="w-full text-white py-3 rounded-full font-medium"
              style={{ background: data.accent }}
            >
              Enviar mensaje
            </button>
      ...
      {/* Mapa fake */}
      <div
        className="aspect-[16/5] relative overflow-hidden border-t"
        style={{ borderColor: `${data.accent}20`, background: `${data.accent}08` }}
      >
        <div
          className="absolute inset-0 opacity-20"
          style={{ background: `radial-gradient(circle at 50% 50%, ${data.accent}, transparent 60%)` }}
        />
        <div className="absolute inset-0 flex items-center justify-center text-foreground/40 gap-2">
      ...
      {/* Volver */}
      <section className="px-6 lg:px-10 py-16 text-center" style={{ background: data.bg }}>
        <Link to="/plantillas" className="link-underline inline-flex items-center gap-2 text-foreground">
          ← Ver todas las plantillas
        </Link>
      </section>
    </div>
